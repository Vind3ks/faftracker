...
function buildSearches(rawQueue) {
  ...
  const searches = Array.from({ length: count }, (_, index) => {
    ...
    return { id: index + 1, label: `Search #${index + 1}`, approximateMean: midpoint(tightBand) ?? midpoint(wideBand), tightBand, wideBand, canMatch: [] };
  });
  ...
  for (const search of searches) {
    search.canMatch = searches
      ...
      .map((candidate) => {
        if (rangesOverlap(search.tightBand, candidate.tightBand)) return { id: candidate.id, label: candidate.label, confidence: "tight", reason: "75% band overlap" };
        if (rangesOverlap(search.wideBand, candidate.wideBand)) return { id: candidate.id, label: candidate.label, confidence: "wide", reason: "80% band overlap" };
        return null;
      })
      .filter(Boolean);
  }
...
function summarizeQueues(rawQueues) {
  ...
  return QUEUES.map((queue) => byKey.get(queue.key) || { key: queue.key, label: queue.label, shortLabel: queue.shortLabel, rawName: queue.aliases[0], ratingType: queue.key, teamSize: Number(queue.shortLabel[0]) || 1, numberOfPlayers: 0, secondsUntilPop: null, popTime: null, searches: [] });
}
...
function getAccessUrl(payload) {
  const candidates = typeof payload === "string" ? [payload] : [payload?.accessUrl, payload?.access_url, payload?.url, payload?.data?.accessUrl, payload?.data?.access_url, payload?.data?.url, payload?.data?.attributes?.accessUrl, payload?.data?.attributes?.access_url, payload?.data?.attributes?.["access-url"]];
...
function safePayloadShape(payload) {
  ...
  return { type: "object", topLevelKeys: Object.keys(payload).slice(0, 20), dataKeys: payload.data && typeof payload.data === "object" ? Object.keys(payload.data).slice(0, 20) : [], attributeKeys: payload.data?.attributes && typeof payload.data.attributes === "object" ? Object.keys(payload.data.attributes).slice(0, 20) : [] };
}
...
async function fetchLobbyAccess(sessionState) {
  ...
  const response = await fetch(`${sessionState.config.userBaseUrl.replace(/\/+$/g, "")}/lobby/access`, {
    headers: { Accept: "application/json", Authorization: `Bearer ${sessionState.tokenSet.accessToken}`, ["X-" + "HMAC"]: sessionState.tokenSet.hmac, "User-Agent": USER_AGENT }
  });
  ...
  if (!response.ok) throw makeError(`Unable to obtain FAF lobby access (HTTP ${response.status}).`, { statusCode: response.status, phase: "lobby-access", detail: payload?.message || payload?.error || payload?.detail || null, hint: "Log in with FAF again or paste a fresh access token from the FAF client." });
  ...
  throw makeError("FAF lobby access response did not contain a usable websocket URL.", { statusCode: 502, phase: "lobby-access-parse", detail: JSON.stringify(safePayloadShape(payload)) });
}
...
function makeUniqueId(sessionId) {
  ...
  if (!uidTool) {
    const fallback = process.env.FAF_ALLOW_FAKE_UID === "1" ? crypto.createHash("sha256").update(`${os.hostname()}:${os.userInfo().username}:${sessionId}:faftracker`).digest("hex") : null;
...
function connectLobby(rawUrl) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let closed = false;
    let closeInfo = null;
    const openTimer = setTimeout(() => { socket.terminate(); reject(makeError("Timed out while opening FAF lobby websocket.", { statusCode: 504, phase: "websocket-open", lobbyUrl })); }, LOBBY_TIMEOUT_MS);
      ...
      resolve({
        ...
        readText(timeoutMs = LOBBY_TIMEOUT_MS, phase = "read") {
          ...
          return new Promise((resolveRead, rejectRead) => {
            ...
            const timer = setTimeout(() => { const index = waiters.indexOf(waiter); if (index >= 0) waiters.splice(index, 1); rejectRead(makeError(`Timed out waiting for FAF lobby response during ${phase}.`, { statusCode: 504, phase, lobbyUrl })); }, timeoutMs);
            waiters.push(waiter);
          });
        },
    ...
    socket.on("error", (error) => { clearTimeout(openTimer); if (!opened) reject(makeError(`Unable to connect to FAF lobby: ${error.message}`, { statusCode: 502, phase: "websocket-open", lobbyUrl })); });
  });
}
...
async function getLiveQueueSnapshot(sessionState) {
  if (!sessionState.tokenSet?.accessToken) throw makeError("Log in with FAF or paste a fresh FAF access token to inspect live queues.", { statusCode: 401, phase: "auth" });
  const info = await requestMatchmakerInfo(sessionState);
  const rawQueues = Array.isArray(info.queues) ? info.queues : [];
  return { ok: true, source: "faf-lobby-matchmaker_info", fetchedAt: new Date().toISOString(), privacyNote: "FAF's lobby only exposes fuzzy live search bands and player counts. It does not expose player names or IDs for queued searches.", matchNote: "Can-match suggestions are approximations based on 75%/80% rating-band overlap. The FAF server still uses its TrueSkill quality calculation when it actually pops the queue.", queues: summarizeQueues(rawQueues), rawQueueCount: rawQueues.length };
}
