...
function logRequest(req, res) {
  ...
  res.on("finish", () => {
    ...
    console.log(JSON.stringify({ level: "info", event: "request", method: req.method, path: pathname, status: res.statusCode, durationMs: Date.now() - startedAt, ip: getClientIp(req), userAgent: String(req.headers["user-agent"] || "").slice(0, 160) }));
  });
}
...
async function handleApi(req, res, url) {
  ...
  if (req.method === "GET" && url.pathname.startsWith("/api/player/")) {
    ...
    if (!provider) return sendJson(res, 404, { error: `Unknown provider "${providerKey}".` });
...
function serveStatic(req, res, url) {
  ...
  if (!resolvedPath.startsWith(PUBLIC_DIR)) return sendText(res, 403, "Forbidden");
...
process.on("unhandledRejection", (error) => console.error(JSON.stringify({ level: "error", event: "unhandledRejection", message: error instanceof Error ? error.message : String(error) })));
process.on("uncaughtException", (error) => console.error(JSON.stringify({ level: "error", event: "uncaughtException", message: error instanceof Error ? error.message : String(error) })));
